...
roman_dict = {'I':1, 'II':2, 'III':3, 'IV':4, 'V':5, 'VI':6, 'T-M3y':999, 'T-3y':999, 'T-4y&o':999,'T-3y&o':999,'T-4y':999,'T-M2y':999,'T-2y':999,'TM3Y':999,'T-M4y':999}
...
def generate_race_stats(df):
    combo = [(3, 3), (4, 3), (5, 4), (3, 5), (4, 5), (3, 2), (4, 2), (5, 3), (3, 4), (4, 4)]
    ...
    for col in mis_col:
        m[col] = [0] * len(m)
    m = m[combo]
    n = df.groupby(["Date", "Venue ID", "Season", "Race No"], as_index=True)['Dr53'].sum()
    o = df.groupby(["Date", "Venue ID", "Season", "Race No"], as_index=True)['Dr54'].sum()
    l = df.groupby(["Date", "Venue ID", "Season", "Race No", "Age"], as_index=True).size().unstack(fill_value=0)
    race_stats = pd.concat([m, n, o, l], axis=1)
    race_stats['Dr53'] = race_stats['Dr53'] / race_stats[(5, 3)]
    race_stats['Dr54'] = race_stats['Dr54'] / race_stats[(5, 4)]
    race_stats['Dr53'] = race_stats['Dr53'].replace([np.nan], [0]).astype(int)
    race_stats['Dr54'] = race_stats['Dr54'].replace([np.nan], [0]).astype(int)
    race_stats.reset_index(inplace=True)
    race_stats = race_stats[["Date", "Venue ID", "Season", "Race No"] + combo + ["Dr53", "Dr54"]]
    race_stats.index = np.arange(1, len(race_stats) + 1)
    race_stats['initial_conditions3'] = race_stats[(5, 3)] & (
                (race_stats[(3, 2)] | race_stats[(4, 2)]) & (race_stats[(3, 4)] | race_stats[(4, 4)])) & (
                                                    race_stats['Dr53'] >= 10)
    race_stats['initial_conditions4'] = race_stats[(5, 4)] & (
                (race_stats[(3, 3)] | race_stats[(4, 3)]) & (race_stats[(3, 5)] | race_stats[(4, 5)])) & (
                                                    race_stats['Dr54'] >= 10)
    return race_stats


def generate_qualifying_races(race_stats):
    qualifying_races = race_stats.loc[race_stats['initial_conditions4'] | race_stats['initial_conditions3']]
...
def generate_qualifying_races_detailed(qualifying_races, df):
    qualifying_races_detailed = pd.merge(qualifying_races, df, on=["Date", "Season", "Race No", "Venue ID"])
    qualifying_races_detailed.set_index(["Date", "Season", "Race No"], inplace=True)
    qualifying_races_detailed.drop(columns=['Dr53', 'Dr54'], inplace=True)
    return qualifying_races_detailed


def generate_qualified_horses(qualifying_races_detailed):
    qualified_horses = qualifying_races_detailed.loc[
        (qualifying_races_detailed['combined_data'] == (5, 3)) | (qualifying_races_detailed['combined_data'] == (5, 4))]
    ...
    qualified_horses = qualified_horses[
        ['Horse', 'Date', 'Season', 'Race No', 'Sr #', 'C-Sr#', 'Trainer', 'Class', 'Distance', 'Age',
         'Wt', 'Jockey', 'Dr', 'Result', 'LBW', 'Penalty', 'Time', 'RunNo.', "Venue ID"]]
...
def generate_TrackedRuns(df, qualified_horses):
    ...
    for i in zerothindex:
        if (i + 1 in races_Q_horses.index):
            if ((races_Q_horses.loc[i + 1, 'Horse'] == races_Q_horses.loc[i, 'Horse']) and abs(
                    races_Q_horses.loc[i + 1, 'Distance'] - races_Q_horses.loc[i, 'Distance']) <= 200 and (
                    races_Q_horses.loc[i + 1, 'Class'] in filtered_allowed_classes) and (
                    roman_dict[races_Q_horses.loc[i + 1, 'Class'].split(' ')[0]] <= roman_dict[
                races_Q_horses.loc[i + 1, 'Class'].split(' ')[0]])):
                races_Q_horses.loc[i + 1, 'RunNo.'] = 1
            elif (races_Q_horses.loc[i + 1, 'Horse'] == races_Q_horses.loc[i, 'Horse'] and races_Q_horses.loc[
                i + 1, 'RunNo.'] != 0 and (
                          abs(races_Q_horses.loc[i + 1, 'Distance'] - races_Q_horses.loc[i, 'Distance']) > 200 or (
                          races_Q_horses.loc[i + 1, 'Class'] not in filtered_allowed_classes) or (
                                  roman_dict[races_Q_horses.loc[i + 1, 'Class'].split(' ')[0]] > roman_dict[
                              races_Q_horses.loc[i + 1, 'Class'].split(' ')[0]]))):
...
filters = [
    ...
    html.Div(
        dcc.Dropdown(
            ...
            options=[{'label': 'Method 2 - WD 34 Sy method', 'value': 'Method 2 - WD 34 Sy method'}],
            value='Method 2 - WD 34 Sy method',
            clearable=False
        ),
...
rules = ["1. This is a method with a decent strike rate, and many times, decent odds. \n", html.Br(),
    "2. This must be treated STRICTLY as a FOLLOW IN NEXT OUTING ONLY method, and should NOT be persisted for later runs in the hope of making profits. \n", html.Br(),
    "3. As the name code suggests, it is about 5y old horses (only), with wide draw number (WD), who finish either 3rd or 4th in a race, and sandwiched between younger horses (3y & 4y) on either side \n", html.Br(),
    "4. The process: Focus ONLY on 5y oldhorses with a wide gate number: 10or higher \n", html.Br(),
    "5. In this method the video watching is required ONLY to check that the horse is galloping with a normal stride action, there are no abnormalities. Nothing else is to be looked at.\n", html.Br(),
    "6. If any 5y old finishes either 3rd or 4th(lengths behind verdict DOES NOT matter, it can be close to the winner or far away), check the age of horses finishing just ahead and just behind \n", html.Br(),
    "7. It means for a 5y old finishing 3rd, check the age of horses finishing 2nd and 4th, and if the 5y old finishes 4th, check the age of horses finishing 3rd and 5th.\n", html.Br(),
    "8. If both the horses on either side ofthe 5y old are YOUNGER (that is, 3yor 4y old), then our 5y old horse qualifies for the NEXT RACE (only)as a bet if and only if:i) it goes out in a race reserved for 5y&older horses,ii) over a trip which is either same or 200m shorter or longer.iii) at the same or lower class level(not higher)iv) ridden by either the same or better jockey (however, the jockey must have ridden winners at the track) \n"]
...
app.layout = html.Div([
    ...
    html.Div(id="store_ytr_result_summary", style={'display': 'none'})],
