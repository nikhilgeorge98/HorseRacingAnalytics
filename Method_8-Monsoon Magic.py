...
def generate_qualified_horses(race_stats, df):
    ...
    qualified_horses = df.loc[(df['Class'].isin(filtered_allowed_classes)) & (df['LBW'] >= 4) & (df['Result'].isin([2, 3, 4])) & (df['Age'] >= 5)][
        ['Horse', 'Date', 'Season', 'Race No', 'Sr #', 'C-Sr#', 'Trainer', 'Rtg', 'Class', 'Distance', 'Age',
...
def generate_TrackedRuns(df, qualified_horses):
    ...
    for i in TrackedRuns.loc[TrackedRuns['RunNo.'] == 1].index:
        # print(i)
        if (TrackedRuns.loc[i, 'split_class'][0] != TrackedRuns.loc[i - 1, 'split_class'][0] or not (
        TrackedRuns.loc[i, 'Class'].endswith('5y&o'))):
            to_drop.append(i)
    # print(to_drop)
