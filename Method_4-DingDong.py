...
def generate_qualified_horses(df):
    qualified_horses = df.loc[(df['Dr'] >= 10) & (df['Distance'] == 1400) & (df['LBW'] <= 0.4) & (df['Result'] == 2)][
...
def generate_TrackedRuns(df, qualified_horses):
    ...
    for i in TrackedRuns.loc[TrackedRuns['RunNo.'] == 1].index:
        if TrackedRuns.loc[i, 'Dr'] >= 10 or (
                TrackedRuns.loc[i, 'Date'] - TrackedRuns.loc[i - 1, 'Date'] > datetime.timedelta(days=42)):
            to_drop.append(i)
...
filters = [
    ...
    html.Div(
        dcc.Dropdown(
            ...
            options=[{'label': 'Method 4 - Ding Dong', 'value': 'Method 4 - Ding Dong'}],
            value='Method 4 - Ding Dong',
            clearable=False
        ),
...
rules = ["1. Applicable only to seven-furlong races (1400m). \n", html.Br(),
    "2. Only horses with wide draw (10 or higher) to be considered. \n", html.Br(),
    "3. Such a horse must do the following during the race: i) Fight a ding-dong battle with the winner in the last part; ii) Lose by neck or less; iii) The live race or video must give the impression that any horse could have won. \n", html.Br(),
    "4. Follow the horse IF AND ONLY IF the horse runs again within 6 weeks of the narrow loss, because such exceptional level of fitness, as displayed when fighting the ding-dong battle, is generally difficult to maintain beyond a month, and nearly impossible to maintain beyond 6 weeks. \n", html.Br(),
    "5. You can follow these horses over any class and with any jockey as long as the jockey has won at the track.\n", html.Br(),
    "6. But make sure the horse MUST have gate position of less than 10. \n", html.Br(),
    "7. As regards the trip distance, our usual rule of plus or minus 200m applies to this method also. It means that you can follow these horses over the same trip (1400m), or 200m smaller (1200m) or 200m longer (1600m).\n"]
