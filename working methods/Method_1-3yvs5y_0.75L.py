...
def generate_qualifying_races(race_stats):
...
def generate_qualified_horses(qualifying_races_detailed):
    ...
    qualified_horses = qualified_horses[
        ['Horse', 'Date', 'Season', 'Race No', 'Sr #', 'Trainer', 'Class', 'Distance', 'Age',
         'Wt', 'Jockey', 'Dr', 'Result', 'LBW', 'Penalty', 'Time', 'RunNo.']]
...
def generate_TrackedRuns(df, qualified_horses):
    ...
    TrackedRuns.loc[
        (TrackedRuns['Result'] != 1) & (TrackedRuns['LBW'] <= 8) & (TrackedRuns['RunNo.'] == 1), 'P1'] = 'P1'
    ...
    TrackedRuns.loc[:, 'RunRes'] = TrackedRuns['W1'] + TrackedRuns['L1'] + TrackedRuns['P1'] + TrackedRuns['W2'] + \
                                   TrackedRuns['L2']
...
def generate_qualified_horse_summary(TrackedRuns):
    ...
    return Run0_df, Run1_df, Run2_df
        ...
        html.Table([
            html.Thead([
                html.Tr([
                    ...
                    html.Th("N-formula", className='overall_place_header')
                ])
            ]),
            html.Tr([
...
def generate_overall_scoreboard_for_combo(TrackedRuns):
    ...
    second_run = TrackedRuns.loc[TrackedRuns['RunNo.'] == 2]
    FTR = len(first_run)
    ...
    STR = len(second_run)
    SW = int(len(second_run.loc[second_run['Result'] == 1]))
    SS = int(len(second_run.loc[second_run['Result'] == 2]))
    ST = int(len(second_run.loc[second_run['Result'] == 3]))
    SP = int(len(second_run.loc[(second_run['Result'] == 1) | (second_run['Result'] == 2) | (second_run['Result'] == 3)]))
    SB = STR - SP
...
def generate_qual_races_details_table(xdf, date, season, race,venue):
    ...
    fav_sb = dash_table.DataTable(
       ...
       'Jockey', 'Dr', 'Result', 'LBW', 'Penalty', 'Time']].to_dict(
        ...
        selected_rows=[],
        css=[{'selector': 'table', 'rule': 'table-layout: fixed'}],
        hidden_columns=['Trainer', 'Jockey', 'Class', 'Distance', 'Wt', 'Dr', 'Penalty', 'Time'],
        style_header={
            'backgroundColor': 'RebeccaPurple',
            'color': 'white',
            'fontWeight': 'bold',
...
def generate_qualified_horse_list_table(xdf):
    ...
    fav_sb = dash_table.DataTable(
       ...
       'Penalty', 'Time']].reset_index().to_dict(
        ...
        selected_rows=[],
        css=[{'selector': 'table', 'rule': 'table-layout: fixed'}],
        hidden_columns=['Trainer', 'Jockey', 'Class', 'Distance', 'Dr', 'Penalty', 'Time'],
        style_header={
            'backgroundColor': 'RebeccaPurple',
            'color': 'white',
            'fontWeight': 'bold',
...
def generate_tracked_runs_selected_horse_table(xdf, horse):
    ...
    if 2 in xdf['RunNo.'].unique():
        ...
        x2 = 'https://www.indiarace.com/Home/racingCenterEvent?venueId={}&event_date={}&race_type=RESULTS'.format(venue2,
                                                                                                                 date2)
        print(x2)
...
filters = [
    ...
    html.Div(
        dcc.Dropdown(
            ...
            options=[{'label': 'Method 1 - 3y vs 5y <0.75L', 'value': 'Method 1 - 3y vs 5y <0.75L'}],
            value='Method 1 - 3y vs 5y <0.75L',
            clearable=False
        ),
...
rules = ["1) This method is about 5y old losing narrowly to 3y old, but under peculiar circumstances. \n", html.Br(),
        "2) The most important thing to watch in the video is that both the 3y & 5y old MUST fight a fierce battle in the last part, in the last 100-200m. \n", html.Br(),
        "3) There should ideally be no third horse close to them, of if it is, then it should always be very clear that the fight is ONLY limited to the 3y & 5y old, and the third horse does not look menacing. \n", html.Br(),
        "4) At the post 3y should win & 5y should finish within less than a length, within 3/4th of a length. \n", html.Br(),
        "5) The strategy to employ here when following the 5y old is very peculiar, in that the 5y old should be followed BLINDLY over any trip or any class/any company in the next run with a full bet\n", html.Br(),
        "6) If it wins, collect your profits. \n", html.Br(),
        "7) If it loses by more than 8 lengths, then assume something is amiss,and drop the horse from your list.\n", html.Br(),
        "8) But if it loses, by a margin of less than 8 lengths, then FOLLOW IT AGAIN NEXT OUT and DOUBLE YOUR BET. Don’t worry about the company or class or jockey. Just a blind double sized bet. \n", html.Br(),
        "9) No need to follow after the first two runs."]
...
row5= dbc.Row(
    [
        dbc.Col(
            ...
            width=12, className='pretty_container twelve columns', id='row5', style={'display': 'initial'}
        )
    ], className='flex-display'
)
...
def update_main(Method, *searchparam):
    ...
    qualifying_races = generate_qualifying_races(race_stats)
    if len(qualifying_races) == 0:
        return [[],'No Qualified Horses', [], {'display': 'none'}, [], [], {'display': 'none'}, [], {'display': 'none'}, [], {'display': 'none'}, [], {'display': 'none'}, [], {'display': 'none'},
                [],[],[],[],[],[],[]]
    ...
    qualified_horses = generate_qualified_horses(qualifying_races_detailed)
    ...
    R0df, R1df, R2df = generate_qualified_horse_summary(TrackedRuns)
    if len(qualifying_races) == 0:
        score_return = "No Qualified Horses"
        # score15_return = []
        # score15_style = {'display': 'none'}
        score2_return = []
        ...
        table5= []
        row5_style = {'display': 'none'}
    else:
        score_return = generate_complete_scoreboard(R0df, R1df, R2df, race_stats, qualifying_races)
        ...
        table4 = generate_horse_run_result_summary_table(R1df, 'table5', 'Run 1')
        row4_style = {'display': 'initial'}
        table5 = generate_horse_run_result_summary_table(R2df, 'table6', 'Run 2')
    ...
    return [html.H2("{} method".format(Method),
        ...
        score_return, score2_return, score2_style, table1left, table1right, row1style, table2, row2_style, table3, row3_style,
            table4, row4_style, table5, row5_style,
