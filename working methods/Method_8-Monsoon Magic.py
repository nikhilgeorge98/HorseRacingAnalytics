...
data['Venue Name'] = data['Venue ID'].replace([1, 2, 3, 4, 7, 8, 9, 10, 11],
                                              ['KOL', 'MUM', 'BLR', 'CHE', 'DEL', 'MYS', 'OOT', 'PUN', 'HYD'])
'''**************************************************************************'''
...
def generate_race_stats(df):
    ...
    race_stats = race_stats[['Date', "Venue ID", 'Season', 'Race No', 'min_horses']]
    return race_stats
...
def generate_qualified_horses(race_stats, df):
    mylist = df.Class.unique()
    r = re.compile("(.+?)4y&o$")
    filtered_allowed_classes = list(filter(r.match, mylist))
    qualified_horses = df.loc[(df['Class'].isin(filtered_allowed_classes)) & (df['LBW'] >= 4) & (df['Result'].isin([2, 3, 4])) & (df['Age'] >= 5)][
        ['Horse', 'Date', 'Season', 'Race No', 'Sr #', 'Trainer', 'Rtg', 'Class', 'Distance', 'Age',
         'Wt', 'Jockey', 'Dr', 'Sh', 'Result', 'LBW', 'Penalty','Time','RunNo.', "Venue ID", "Centre"]]
    ...
    for i in zerothindex:
        if (i + 1 in races_Q_horses.index):
            if (races_Q_horses.loc[i + 1, 'Horse'] == races_Q_horses.loc[i, 'Horse'] and
                    races_Q_horses.loc[i + 1, 'split_class'][0] == races_Q_horses.loc[i, 'split_class'][0] and (
                    races_Q_horses.loc[i + 1, 'Class'].endswith('5y&o'))):
                races_Q_horses.loc[i + 1, 'RunNo.'] = 1
            elif (races_Q_horses.loc[i + 1, 'Horse'] == races_Q_horses.loc[i, 'Horse'] and races_Q_horses.loc[
                i + 1, 'RunNo.'] != 0 and (
                          races_Q_horses.loc[i + 1, 'split_class'][0] != races_Q_horses.loc[i, 'split_class'][
                      0] or not (races_Q_horses.loc[i + 1, 'Class'].endswith('5y&o')))):
...
def generate_qualified_horse_list_table(xdf):
    ...
    fav_sb = dash_table.DataTable(
       ...
       'Penalty', 'Time', "Venue ID"]].reset_index().columns],
        data=xdf[
            ['Horse', 'Date', 'Season', 'Race No', 'Sr #', 'Trainer', 'Class',
       'Distance', 'Age', 'Wt', 'Jockey', 'Dr', 'Result', 'LBW',
       'Penalty', 'Time', "Venue ID"]].reset_index().to_dict(
            'records'),
        editable=False,
        export_columns='all',
        export_format='xlsx',
...
filters = [
    ...
    html.Div(
        dcc.Dropdown(
            ...
            options=[{'label': 'Method 8 - Monsoon Magic', 'value': 'Method 8 - Monsoon Magic'}],
            value='Method 8 - Monsoon Magic',
            clearable=False
        ),
...
rules = ["1) The method MUST be used ONLY during the Summer/Monsoon seasons which start after a longish gap after the end of Winter seasons. \n", html.Br(),
        "2) The method is restricted ONLY to horses in classes IV and V. \n", html.Br(),
        "3) Focus ONLY on horses 5y or older. \n", html.Br(),
        "4) The qualifying race should be for horses 4y&over \n", html.Br(),
        "5) We will pick up any 5y or older horse which finishes either 2nd or 3rd or 4th, but AT LEAST 4 lengths BEHIND the winner.\n", html.Br(),
        "6) Note: There should be at least 9 (ideally, 10 or more) runners in the qualifying race. That way we can be sure our horse has beaten a number of runners. \n", html.Br(),
        "7) Immediate next out, this horse MUST enter a 5y&over field IN THE SAME CLASS as the qualifying race. It will NOT qualify even if it goes to lower class. ONLY SAME CLASS, but 5y&older set this time.\n", html.Br(),
        "8) Ignore distance in the bettable run, but make sure the odds are in single digit, less than 10/1. \n", html.Br(),
        "9) The jockey in the bettable race should be a winner or place getter at least occasionally, SKIP if the jockey just is not capable of winning.", html.Br(),
        "10) This is a FOLLOW IN NEXT OUTING  ONLY method. Avoid all temptation to follow again. It will NOT pay in the long run.", html.Br(),
        "11) Also, scrap the list of these horses on Sept 30. This is a strictly May-Sept method." ]
...
def update_main(Method, *searchparam):
    ...
    qualified_horses = generate_qualified_horses(race_stats, df)
