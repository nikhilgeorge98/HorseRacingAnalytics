...
def generate_qualified_horses(df):
    qualified_horses = df.loc[(df['Dr'] >= 10) & (df['Distance'] == 1200) & (df['LBW'] <= 0.4) & (df['Result'] == 2)][
...
def generate_TrackedRuns(df, qualified_horses):
    ...
    for i in zerothindex:
        if (i + 1 in races_Q_horses.index):
            if (races_Q_horses.loc[i + 1, 'Horse'] == races_Q_horses.loc[i, 'Horse'] and races_Q_horses.loc[i + 1, 'Dr'] < 10 and (
                    races_Q_horses.loc[i + 1, 'Date'] - races_Q_horses.loc[i, 'Date'] < datetime.timedelta(days=42))):
                races_Q_horses.loc[i + 1, 'RunNo.'] = 1
            elif (races_Q_horses.loc[i + 1, 'Horse'] == races_Q_horses.loc[i, 'Horse'] and (
                    races_Q_horses.loc[i + 1, 'Dr'] >= 10 or (
                    races_Q_horses.loc[i + 1, 'Date'] - races_Q_horses.loc[i, 'Date'] >= datetime.timedelta(days=42)))):
