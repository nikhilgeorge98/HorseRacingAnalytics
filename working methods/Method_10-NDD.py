...
def tracked_runs(trackedRuns_json):
    try:
        ...
        raise PreventUpdate
    Run0_df, Run1_df, Run2_df = generate_qualified_horse_summary(TrackedRuns)
    print("Run1", len(Run1_df), "run2", len(Run2_df))
    return [Run0_df.to_json(orient='split')], [Run1_df.to_json(orient='split')], [Run2_df.to_json(orient='split')]
...
def update_main(dff,race_stats_json,init_qual_races_json,init_qual_races_detailed_json,init_qual_horses_json,qual_races_json,qual_races_detailed_json,qual_horses_json,trackedRuns_json, run0_json, run1_json, run2_json, run1_overall,run2_overall, Method):
    ...
    if len(qualifying_races) == 0:
        return [[],'No Qualified Horses', [], {'display': 'none'}, [],[], [], {'display': 'none'}, [], {'display': 'none'}, [], {'display': 'none'}, [], {'display': 'none'}, [], {'display': 'none'}, {'display': 'none'}, [], {'display': 'none'}, []]
    try:
        qualifying_races['Date'] = qualifying_races['Date'].dt.date
    ...
    return [html.H2("{} method".format(Method),
        ...
        score_return, score2_return, score2_style, score25_return1, table1left, table1right, row1style, table2, row2_style, table3, row3_style,
            table4, row4_style, table5, row5_style, row3point5style, scorerun1, row4point5style, scorerun2]
