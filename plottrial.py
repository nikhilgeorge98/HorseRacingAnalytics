...
fields = ["overall_update","time", "forward angle", "sideways angle", "direction string", "updated by", "count for thread", "processing time"]
        ...
        while cv2.getWindowProperty(previewName, 0) >= 0:
            ...
            if len(corners) > 0:
                ...
                if (len(centroids) > 1):
                    ...
                    message = direction + ",{}".format(datetime.now().time())
                    rowtowrite = [overall_update, "'"+str(datetime.now().time()), forwardangle, sidewaysangle, direction, threadname, count, exectime]
                    print(message)
