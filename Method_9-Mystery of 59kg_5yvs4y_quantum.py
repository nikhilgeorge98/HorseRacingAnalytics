...
def generate_race_stats(df):
    ...
    race_stats = race_stats[['Date', "Venue ID", 'Season', 'Race No']]
    return race_stats
...
def generate_qualified_horses(race_stats, df):
    ...
    qualified_horses = df.loc[(df['Wt'] == 59) & (df['LBW'] >= 8) & (df['LBW'] <= 9) & (df['Age'] == 5) & (
        df['Class'].isin(filtered_allowed_classes))][
...
filters = [
    ...
    html.Div(
        dcc.Dropdown(
            ...
            options=[{'label': 'Method 9 - Mystery of 59kg(5y vs 4y)', 'value': 'Method 9 - Mystery of 59kg(5y vs 4y)'}],
            value='Method 9 - Mystery of 59kg(5y vs 4y)',
            clearable=False
        ),
...
rules = ["1. I will use this method to explain how I actually went about researching this insight. \n", html.Br(),
        "2. Applicable only to horses carrying actual 59 kg on the back, after taking into account overweight or allowance claimed. \n", html.Br(),
        "3. If you must know, 59 kg in the British system is 9 St 4 Lb (130 Lb). \n", html.Br(),
        "4. In my quantum LBW approach, the figure of 59 is related to a range of 2.8 to 3 (it’s some complicated calculation which is beyond the scope of this discussion), so first I checked all horses that had either won or lost by LBW in that range. But I couldn’t find any connection. \n", html.Br(),
        "5. Then, with sheer luck, just a thought occurred to me to check the squared range of 2.8 to 3, and I found it is exactly between 8 & 9 lengths.\n", html.Br(),
        "6. I was shocked when I checked for 5y old horses having run against 4y&o (yesterday’s Monsoon Magic discussion) and lost by between 8 to 9 lengths: it threw up 5 cases, 2 of which did not qualify as per our standard process, and ALL THE REMAINING THREE HAD WON IN NEXT START. \n", html.Br(),
        "7. However, as Dynamic Handicappers, we should not get carried away by this revelation. We still do not know if it is a case of coincidence, correlation or causation.\n"]
