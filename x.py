...
def camPreview(previewName, camID, lock):
    global direction
            ...
            if len(corners) > 0:
                ...
                if (len(centroids) > 1):
                    ...
                    if previewName == "Camera 2":
                        output = cv2.line(image, centroids[4], centroids[8], color, 2)
                        # output = cv2.line(image, centroids[2], centroids[4], color, 2)
                        # output = cv2.line(image, centroids[4], centroids[6], color, 2)
                        output = cv2.line(image, centroids[4], centroids[8], (0, 255, 0), 2)

                    else:
                        output = cv2.line(image, centroids[1], centroids[2], color, 2)
                        # output = cv2.line(image, centroids[2], centroids[4], color, 2)
                        # output = cv2.line(image, centroids[2], centroids[6], color, 2)
                        output = cv2.line(image, centroids[1], centroids[2], (0, 255, 0), 2)
                        angle3 = round(angle2([centroids[1], centroids[2]]), 2)
                        if angle3 < 80:
                            if angle3 < 70:
                                sideways = "R2"
                            else:
...
column_names = ["frame", "sideways angle", "forward angle"]
df = pd.DataFrame(columns = column_names)
df['frame']=np.arange(1,10000)
vid1 = "108322073138"
vid2 = "108322073108"
lock = Lock()
...
thread2.join()
# print("Finished!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!")
# df.dropna(subset=['sideways angle','forward angle'], how='all' ,inplace=True)
# df.to_csv(r'C:/Users/Admin/PycharmProjects/pythonProject1/doublenew.csv', index = False)